...
import string, sys, copy, lib
from atom import Atom
from conformation_container import Conformation_container
...
def read_pdb(pdb_file, parameters, molecule):
    ...
    names = sorted(conformations.keys(), key=lib.conformation_sorter)
...
def get_atom_lines_from_pdb(pdb_file, ignore_residues = [], keep_protons=False, tags = ['ATOM  ', 'HETATM'], chains=None):
    ...
    lines = lib.open_file_for_reading(pdb_file).readlines()
...
def write_pdb_for_atoms(atoms, filename, make_conect_section=False):
    out = lib.open_file_for_writing(filename)
...
def get_atom_lines_from_input(input_file, tags = ['ATOM  ','HETATM']):
    lines = lib.open_file_for_reading(input_file).readlines()
    conformation = ''
